...
describe("the row being typed into stays visible", () => {
  test("the keyboard opening reveals the caret row and closing leaves scroll alone", async () => {
    const main = await Bun.file(new URL("./bootstrap.ts", import.meta.url)).text();
    const term = await Bun.file(new URL("../features/session/guided/term.ts", import.meta.url)).text();
    expect(main).toContain("if (!open ||");
    expect(main).toContain("schedulePageRaf(revealCaretRow)");
  ...
  test("the dock settle rides on the settled state, never on the shell height", async () => {
    const dock = await Bun.file(new URL("../features/session/guided/dock.scss", import.meta.url)).text();
    const shell = await Bun.file(new URL("../features/session/guided/session-shell.scss", import.meta.url)).text();
