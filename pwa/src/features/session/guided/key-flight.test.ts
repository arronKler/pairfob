...
const keys = await Bun.file(new URL("./keys.ts", import.meta.url)).text();
const fullTerminal = await Bun.file(new URL("../full-terminal/full-terminal.ts", import.meta.url)).text();
const styles = await Bun.file(new URL("./key-flight.scss", import.meta.url)).text();
...
describe("a pressed key shows where it went", () => {
  ...
  test("keys the terminal will not echo are marked and named", () => {
    ...
    expect(styles).toContain(".key-spark.is-silent");
  ...
  test("reduced motion drops the travel but keeps the caret pulse and label", () => {
    ...
    expect(styles).toMatch(/prefers-reduced-motion: reduce\)\s*\{[\s\S]*\.key-spark \{ display: none; \}/);
  });
