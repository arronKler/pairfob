...
/** A newer same-daemon activate() owns the slot; the older transport is closed. */
export class SupersededActivationError extends Error {
  constructor(daemonId: string) {
    super(`superseded activation for ${daemonId}`);
 ...
 *
 * A per-daemon activation epoch makes the *latest* activate() own the slot:
 * when two activations for one daemon overlap, the older one's transport is
 * closed instead of overwriting the newer entry. Different daemons activate
 * concurrently and never queue behind each other.
 */
export class ComputerSessions {
  ...
  async activate(pair: PairResult, connect: SessionConnector): Promise<ActivatedSession> {
    const epoch = (this.epochs.get(pair.daemonId) ?? 0) + 1;
    this.epochs.set(pair.daemonId, epoch);
    ...
    connected = session;
    if (this.epochs.get(pair.daemonId) !== epoch) {
      // A newer activation for this daemon owns the slot. Never insert the
      // superseded transport over it and never let it evict a current entry.
      session.close();
      throw new SupersededActivationError(pair.daemonId);
    }
