...
/**
 * A press mark that starts where the finger landed, so a tap on a wide control
 * reads as "here", not "somewhere in this rectangle". For an element that
 * outlives a repaint; the app shell uses bindRippleSurface instead.
 */
export function bindRipple(target: HTMLElement): void {
  target.classList.add("ripple-host");
  target.addEventListener("pointerdown", (event) => {
    if (event.button === 0) spawnRipple(target, event.clientX, event.clientY);
  });
}

/**
 * Screens here repaint through replaceChildren, so per-element binding would
 * have to be redone on every paint. One delegated listener on the document
 * covers the standard actions instead, dialogs included.
 */
