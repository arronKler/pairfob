...
import { FullTerminalStateLayer } from "./react/full-terminal-state-layer";
...
const { app } = await import("../state.ts");
...
function paint() {
  act(() => {
    renderReactScreen(createElement(FullTerminalStateLayer, { onRetry: () => retries++ }));
  });
}
...
describe("full terminal centered state", () => {
  test("shows a polite loading state with prominent detail", () => {
    ...
    expect(app.querySelector(".full-terminal-state-title")?.textContent).toBe("正在载入会话终端");
    expect(app.querySelector(".full-terminal-state-detail")?.textContent).toContain("建立加密连接");
    expect(app.querySelector<HTMLButtonElement>(".full-terminal-state-retry")?.hidden).toBe(true);
  });
  ...
  test("announces an error and exposes a working retry button", () => {
    ...
    const layer = app.querySelector<HTMLElement>(".full-terminal-state")!;
    const retry = app.querySelector<HTMLButtonElement>(".full-terminal-state-retry")!;
    expect(layer.getAttribute("role")).toBe("alert");
    expect(layer.getAttribute("aria-live")).toBe("assertive");
    expect(app.querySelector(".full-terminal-state-title")?.textContent).toBe("无法打开会话终端");
