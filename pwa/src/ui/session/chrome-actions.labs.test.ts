...
for (const key of ["window", "document", "location", "history", "HTMLElement", "HTMLButtonElement", "Node"] as const) {
  globals[key] = (happy as unknown as Record<string, unknown>)[key];
}

const { chromeActionCluster } = await import("./chrome-actions.ts");

describe("workspace labs entry", () => {
  test("keeps the existing chrome unchanged without the URL flag", () => {
  ...
  test("adds the workspace action before more with labs=workspace", () => {
    history.replaceState(null, "", "/pair?labs=workspace");
    const cluster = chromeActionCluster(() => {}, () => {});
