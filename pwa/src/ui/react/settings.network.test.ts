...
import { NETWORK_MODE_KEY, app, clearNotice, setNetworkMode, state } from "../../state";
...
describe("settings network transport", () => {
  ...
  test("disables P2P when the origin kill switch is off", () => {
    ...
    paint();
    const p2p = [...group().querySelectorAll("button")].find((item) => item.textContent === "P2P");
    const auto = [...group().querySelectorAll("button")].find((item) => item.textContent === "自动");
    const relay = [...group().querySelectorAll("button")].find((item) => item.textContent === "Relay");
    expect((p2p as HTMLButtonElement).disabled).toBeTrue();
    expect((auto as HTMLButtonElement).disabled).toBeFalse();
    expect((relay as HTMLButtonElement).disabled).toBeFalse();
