...
const { FullTerminalScreen } = await import("./full-terminal.tsx");
...
function boot(): void {
  ...
  state.live = live();
  setRenderer(() => renderFullTerminal(() => undefined, () => undefined, () => undefined));
  renderFullTerminal(() => undefined, () => undefined, () => undefined);
}
...
describe("react complete-terminal shell", () => {
  ...
  test("async error and retry ARIA come from the view store", async () => {
    ...
    act(() => renderFullTerminal(() => undefined, () => undefined, () => undefined));
  ...
  test("FullTerminalScreen is the route component", () => {
    state.paneId = "p1";
    state.fullTerminal = true;
    act(() => renderReactScreen(createElement(FullTerminalScreen, {
      onBack: () => undefined,
    ...
    expect(app.querySelector("[data-react-full-terminal]")).toBeTruthy();
  });
});
