...
afterEach(() => act(leaveReactScreen));
...
test("completed turns retain their process fold, Markdown and one final reply copy action", () => {
  const copied: string[] = [];
  act(() => renderReactScreen(<AgentStream items={items} working={false} empty={empty} onCopyReply={text => { copied.push(text); }} />));
  expect(app.querySelector(".agent-user-text")?.textContent).toBe(items[0].text);
  expect(app.querySelector<HTMLDetailsElement>(".agent-reply-fold")!.open).toBeFalse();
  expect(app.querySelectorAll(".agent-step")).toHaveLength(2);
  expect(app.querySelector(".agent-md h2")?.textContent).toBe("Result");
  expect(app.querySelector(".agent-md strong")?.textContent).toBe("Safe");
  const link = app.querySelector(".agent-md a")!;
  expect(link.getAttribute("rel")).toBe("noopener noreferrer");
  expect(link.getAttribute("target")).toBe("_blank");
  expect(app.querySelectorAll(".agent-reply-copy")).toHaveLength(1);
  act(() => app.querySelector<HTMLButtonElement>(".agent-reply-copy")!.click());
...
test("manual collapse of a live process survives new output and preserves its details node", () => {
  const paint = (text: string) => act(() => renderReactScreen(<AgentStream items={[...items.slice(0, 3), { type: "assistant", text }]}
    working empty={empty} />));
  paint("first partial reply");
  const process = app.querySelector<HTMLDetailsElement>(".agent-process")!;
  expect(process.open).toBeTrue();
  toggle(process, false);
  expect(process.dataset.user).toBe("closed");
  paint("second partial reply");
  expect(app.querySelector(".agent-process") === process).toBeTrue();
  expect(process.open).toBeFalse();
  expect(app.querySelector(".agent-run-status")?.textContent).toContain(t("trace.runningSteps", { n: 3 }));
  expect(app.querySelector(".agent-reply-copy")).toBeNull();
});
...
test("lazy tool details wait for expansion, render loading/error and retry only by user action", async () => {
  ...
  const view = () => detail;
  const paint = () => act(() => renderReactScreen(<AgentStream items={[{ type: "user", text: "run" }, tool]}
    working empty={empty} toolDetail={view} onNeedToolDetail={need} />));
  paint();
  await act(async () => { await Promise.resolve(); });
  expect(requests).toEqual([]);
  const step = app.querySelector<HTMLDetailsElement>(".agent-step")!;
  toggle(step, true);
...
test("duplicate prompts remain separate turns and unsafe Markdown cannot introduce active elements", () => {
  ...
  act(() => renderReactScreen(<AgentStream items={turns} working={false} empty={empty} />));
  expect(app.querySelectorAll(".agent-user")).toHaveLength(2);
  expect(app.querySelectorAll(".agent-assistant-final")).toHaveLength(2);
...
test("error retry and scroll requests use the existing near-top and follow thresholds", () => {
  ...
  act(() => renderReactScreen(<AgentStream items={[]} working={false} busy empty={{ kind: "error", title: "Read failed" }}
  ...
  act(() => app.querySelector<HTMLButtonElement>(".agent-empty button")!.click());
