...
const { resetComposeDrafts } = await import("../../compose-drafts");
...
function boot(live = session(), composeLive = false): void {
  state.phase = "live";
  state.screen = "pane";
  state.paneId = "p1";
  state.live = live;
  ...
  state.composeLive = composeLive;
  state.runtimeKind = "herdr";
  state.networkOnline = true;
  state.agents = [{ paneId: "p1", agent: "herdr", hasAgent: true, status: "working", workspaceLabel: "demo", cwd: "/tmp/demo" }];
...
beforeEach(async () => {
  ...
  await act(() => { disposeFullTerminal(); leaveReactScreen(); });
  resetComposeDrafts();
...
describe("independent complete-terminal React review", () => {
  ...
  test("mode changes keep the engine and send draft once without replay on repaint", async () => {
    const live = session();
    state.composeDraft = "send once";
    await act(() => boot(live));
    await until(() => getFullTerminalView().stage === "live", "live terminal");
    const host = app.querySelector(".full-terminal-host");
    const helper = app.querySelector(".xterm-helper-textarea");
    await act(() => setFullTerminalComposeLive(true));
    await act(() => { renderApp(); syncFullTerminalChrome(); });
