...
import { echoGhost, echoStoreRevision, subscribeEcho } from "../session/echo";
import { openRow } from "../session/rowbar";
import {
  atBottom,
  bindPinch,
  bindTap,
...
import { unreadBars, unreadCount } from "../session/unread";
import { bindHostScroll } from "../full-terminal-scroll";
...
function JumpChip({ jumpRef }: { jumpRef: { current: HTMLButtonElement | null } }) {
  ...
  const hidden = leaving ? false : state.paneFollow || !state.paneUnread;
...
export function SessionTerminal({ onRow }: { onRow?: (index: number) => void } = {}) {
  ...
  const ghost = echoGhost(state.paneId ?? "");
  ...
  const termClass = `term${state.termWrap ? " wrapped" : ""}${state.termSelect ? " selecting" : ""}`;
  return (
