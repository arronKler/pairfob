...
<div class="pf-claims">
  <article>
    ...
    <p>读电脑已经画好的 pane，按键打回本机 PTY。手机上点的确认、打的字、开的分屏，电脑上立刻就是；反过来也一样。</p>
  </article>
  <article>
  ...
  <article>
    <p class="pf-claim-t">CLI 有的才画</p>
    <p>新建、分屏、worktree、控制 / 终端 / 对话，都以正在跑的 Herdr 为准。电脑当时做不到的，手机上不会出现，也不会假装成功。</p>
  </article>
  <article>
    <p class="pf-claim-t">家里不开端口</p>
    <p>pairfobd 只出站连 relay。不绑 Tailscale，不把 Herdr 暴露到公网。配对完成后，内容端到端加密。</p>
  </article>
</div>
...
<div class="pf-vs">
  ...
  <div class="pf-vs-row">
    <p class="pf-vs-not" data-not="不是">浏览器里再画一遍 xterm</p>
    <span class="pf-vs-arrow" aria-hidden="true">→</span>
    <p class="pf-vs-is">读已经渲染好的 pane</p>
  </div>
  <div class="pf-vs-row">
  ...
  <div class="pf-vs-row">
    ...
    <span class="pf-vs-arrow" aria-hidden="true">→</span>
    <p class="pf-vs-is">电脑当时的 CLI 能力原样保留</p>
  </div>
  <div class="pf-vs-row">
  ...
  <div class="pf-vs-row">
    ...
    <span class="pf-vs-arrow" aria-hidden="true">→</span>
    <p class="pf-vs-is">电脑主动出站，家里不用开端口</p>
  </div>
</div>

更展开的产品模型见 [同一块屏幕](/zh/model)，链路见 [怎么接起来](/zh/architecture)。

## 适合谁

电脑上已经在用 Herdr 0.7 或更高版本跑 coding agent，又希望：

- 离开工位时用系统键盘接着写、确认对话框、开 worktree
- 坐回来什么都不用同步
- 家里路由器不用做端口转发，也不想为这个去绑远程网络

目前支持 **macOS 和 Linux**。Windows 还不能装 `pairfobd`。
...
<ol class="pf-track">
<li><p>电脑上装好 Herdr；pairfobd 启动时会拉起默认持久 server</p></li>
<li><p><a href="./install">安装</a> <code>pairfobd</code>：<code>curl -fsSL https://pairfob.com/install.sh | sh</code></p></li>
<li><p>电脑执行 <code>pairfobd pair</code>，另一台设备打开 <a href="/pair">pairfob.com/pair</a></p></li>
<li><p>手机证明配对码后，在电脑终端按一次 Enter 确认</p></li>
</ol>
