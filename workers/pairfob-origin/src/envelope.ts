// Duplicated pairfob.v1 24-byte header (Workers cannot import the PWA codec).
// FWD payload is opaque: this module never interprets bytes after the header.
...
export function knownType(typ: number): boolean {
  return KNOWN_TYPES.has(typ);
}
...
export function validateFrame(frame: Frame): void {
  ...
  if (!knownType(frame.typ)) throw new Error(`unknown envelope type 0x${frame.typ.toString(16)}`);
...
export function decode(bytes: Uint8Array): Frame {
  ...
  if (length > MAX_PAYLOAD) throw new Error(`length > ${MAX_PAYLOAD}`);
  if (length + HEADER_SIZE !== bytes.length) throw new Error("length mismatch");
  const frame: Frame = {
    version: bytes[0],
    typ: bytes[1],
    flags: view.getUint16(2, false),
    // Keep views into the validated WebSocket message. Relay FWD can then
    // forward the original wire buffer instead of copying an opaque payload.
    routeId: bytes.subarray(8, 24),
